...
class liionpack_actor:
    def __init__(self, **kwargs):
        ...
        integrator, variables_fn, t_eval = cco(I_init, htc_init,
                                               self.simulation, dt, Nspm,
                                               nproc, variable_names,
                                               mapped)
    ...
    def step(self, dt=10, inputs=None):
        step_solutions, var_eval = ss(self.simulation.built_model,
                                      self.step_solutions,
                                      inputs,
                                      self.integrator,
                                      self.variables_fn,
...
if __name__ == "__main__":
    ...
    for i in range(Nworkers):
        pa = client.submit(
            liionpack_actor, actor=True, pure=False, chemistry=chemistry,
            I_init=I_app, htc_init=htc, dt=dt, Nspm=spm_per_worker
        )
        futures.append(pa)
