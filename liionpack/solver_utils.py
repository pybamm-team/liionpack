...
def _serial_step_dask(model, solutions, inputs_dict, integrator, variables, t_eval):
    """
    Internal function that uses Dask to process the model for one timestep in
    a serial way.
    """
    len_rhs = model.concatenated_rhs.size
    N = len(solutions)
...
def solve_dask(
    ...
    # Setup client to use Dask distributed scheduler
    client = Client()
    print(client)
    ...
    for step in range(Nsteps):
        ...
        inputs = lp.build_inputs_dict(shm_i_app[step, :], htc)

        step_solutions, var_eval = _serial_step_dask(
            sim.built_model, step_solutions, inputs, integrator, variables_fn, t_eval)
        ...
        # Check lower and upper voltage limits
        if np.any(temp_v < v_cut_lower):
            print('Low V limit reached')
            break
