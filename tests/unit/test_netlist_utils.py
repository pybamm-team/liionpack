...
class netlist_utilsTest(unittest.TestCase):
    ...
    def test_circuit_configuration(self):
        netlist = lp.setup_circuit(Np=2, Ns=3, Rb=1e-4, Rc=1e-2, Ri=1e-3, V=2.0, I=10.0, configuration="parallel-strings")
        assert sum(netlist["desc"].str.find("Rb") > -1) == 2
        netlist = lp.setup_circuit(Np=2, Ns=3, Rb=1e-4, Rc=1e-2, Ri=1e-3, V=2.0, I=10.0, configuration="series-groups")
    ...
    def test_solve_circuit_seriesgroups(self):
        netlist = lp.setup_circuit(Np=1, Ns=2, Rb=1e-4, Rc=1e-2, Ri=1e-3, V=2.0, I=1.0, configuration="series-groups")
        V_node, I_batt = lp.solve_circuit(netlist)
        assert np.all(I_batt) == 1.0
