...
def parametric_ocp(sto, p):
    ...
    return (
        p[0] + p[1] * sto + p[2] * np.exp(p[3] * sto) + p[4] * np.exp(p[5] * (1 - sto))
    )
...
print(
    f"k = {p_delithi[0]:.4f} + ({p_delithi[1]:.4f} * sto) + ({p_delithi[2]:.4f} * np.exp(c1)) + ({p_delithi[4]:.4f} * np.exp(c2))"
)
...
print(
    f"k = {p_delithi[0] - 0.06:.4f} + ({p_delithi[1]:.4f} * sto) + ({-p_delithi[4]:.4f} * np.exp(c1)) + (-{p_delithi[2]:.4f} * np.exp(c2))"
)
...
plt.scatter(
    pts,
    LFP_ocp_phase_field(np.array(pts)),
    color="lime",
    zorder=10,
    label="Constraints",
    edgecolors="black",
