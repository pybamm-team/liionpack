...
def graphite_ocp_avg(sto):
    ...
    width = 5e-2
    muLtail = -5e-2 * 1.0 / (sto ** (0.85))
    muRtail = 5e-2 * 1.0 / ((1.001 - sto) ** (0.85))
...
def graphite_ocp_delithi(sto):
    ...
    muR_bias = (
        -0.3
        * 0.5
        * (-np.tanh((sto - 0.5) / (0.4 * width)) + 1)
        * 0.5
        * (np.tanh((sto - 0.25) / (0.4 * width)) + 1)
    )
...
def graphite_ocp_phase_field(sto):
    ...
    muLlin = (
        ...
        * (0.40 - sto**0.98)
        * 0.5
        * (-np.tanh((sto - 0.49) / (0.9 * width)) + 1)
        * 0.5
        * (np.tanh((sto - 0.35) / width) + 1)
    )
    muRlin = (
        (0.1 * Omga * 4 * (0.74 - sto) + 0.90 * Omgb)
        * 0.5
        * (np.tanh((sto - 0.5) / (0.4 * width)) + 1)
    )
    muLMod = (
...
def LFP_ocp_avg(sto):
    ...
    k = (
        (3.4075 + 3.4623) / 2
        - 0.01 * sto
        + (0.0787 + 0.0668) / 2 * np.exp(c1)
        - (0.0787 + 0.0668) / 2 * np.exp(c2)
    )
    return k
...
if __name__ == "__main__":
    ...
    plt.plot(
        sto,
        graphite_ocp_phase_field(sto),
        linestyle="--",
        color="black",
        label="Graphite OCP phase field",
    )
    plt.ylim(0, 0.2)
    plt.xlim(0, 1)
