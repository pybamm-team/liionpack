...
def run_pack(
    ...
    (total_current_in_each_section, termination_conditions, _, dt) = (
        generate_protocol_from_experiment(experiment)
    )
    ...
    for i in range(num_cells_parallel):
        if initial_soc is not None:  # if inital soc is given.
            sims[i].build(
                ...
                inputs={
                    "Current function [A]": total_current_in_each_section[0][0]
                    / num_cells_parallel
                },
            )
    ...
    for s in range(len(total_current_in_each_section)):
        ...
        section_current = next(
            (val for val in section_profile if abs(val) > 1e-12), 0.0
        )
        ...
        if section_termination is not None:
            if section_termination["type"] == "pack_voltage":
                if section_current < 0:
                    print(
                        f"Termination: charge until pack terminal voltage reaches {section_termination['value']} V"
                    )
                    v_cut_higher = section_termination["value"]
                elif section_current > 0:
                    print(
                        f"Termination: discharge until pack terminal voltage reaches {section_termination['value']} V"
                    )
        ...
        for t in range(len(section_profile)):
            ...
            print(
                f"Time step {t + 1}/{len(total_current_in_each_section[s])} at t={updated_time:.2f}s",
                end="\r",
            )
            ...
            for i in range(num_cells_parallel):
                if save_memory:
                    ...
                    sims[i].step(
                        dt=dt, save=True, inputs={"Current function [A]": currents[i]}
                    )
                ...
                history["T_cell"][i].append(
                    sims[i].solution["Cell temperature [C]"].entries[-1]
                )
