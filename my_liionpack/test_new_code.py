...
def plot_results(history, num_cells):
    ...
    for i in range(num_cells):
        axes[0].plot(
            time + time[1], history["I_cell"][i], label=f"Cell {i + 1}", color=f"C{i}"
        )
    ...
    for i in range(num_cells):
        axes[1].plot(
            time + time[1], history["V_cell"][i], label=f"Cell {i + 1}", color=f"C{i}"
        )
    ...
    for i in range(num_cells):
        axes[2].plot(
            time, history["R_internal"][i], label=f"Cell {i + 1}", color=f"C{i}"
        )
...
for i in range(num_cells_parallel):
    ax[0].plot(
        t, currents[:, i], "--", label="Liionpack", alpha=0.7, color="C" + str(i)
    )
...
plt.figure(figsize=(8, 5))
plt.plot(
    history["time"], history["V_pack"], linewidth=2, label="Pack voltage (my model)"
)
plt.plot(
    out["Time [s]"],
    out["Pack terminal voltage [V]"],
    "--",
    linewidth=2,
    label="Pack voltage (liionpack)",
)
